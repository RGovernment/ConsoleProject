...
{
    /// <summary>
    /// 키와 값이 일치하는 JObject 반환, index는 반환하지 않음
    /// </summary>
    /// <param name="jArray"></param>
    /// <param name="key"></param>
    /// <param name="value"></param>
    /// <returns></returns>
    public static JObject FindJObjectByKeyAndValueOnlyObj(JArray jArray, string key, string value)
    {
        foreach (JObject obj in jArray.Cast<JObject>())
        {
            if (obj.ContainsKey(key) && obj[key].ToString().Equals(value))
            {
                return obj;
            }
        }
    ...
    /// <param name="value"></param>
    /// <returns></returns>
    public static bool FindJObjectByKeyAndValueOnlyObj(JArray jArray, string key, string value, out JObject result)
    {
        foreach (JObject obj in jArray.Cast<JObject>())
        {
            if (obj.ContainsKey(key) && obj[key].ToString().Equals(value))
    ...
    /// <summary>
    /// 키와 값이 일치하는 JObject와 해당 Object의 index 값 반환
    /// </summary>
    /// <param name="jArray"></param>
    /// <param name="key"></param>
    /// <param name="value"></param>
    /// <returns></returns>
    public static (JObject, int) FindJObjectByKeyAndValue(JArray jArray, string key, string value)
    {
        int count = 0;
        foreach (JObject obj in jArray.Cast<JObject>())
        {
    ...
    /// <summary>
    /// 키와 값이 일치하는 JObject 존재 여부 반환
    /// </summary>
    /// <param name="jArray"></param>
    /// <param name="key"></param>
    /// <param name="value"></param>
    /// <returns></returns>
    public static bool FindBoolJObjectByKeyAndValue(JArray jArray, string key, string value)
    {
        int count = 0;
        foreach (JObject obj in jArray.Cast<JObject>())
        {
    ...
    /// <returns>값을 찾을 경우 false, 찾지 못할 경우 true *헷갈릴 수 있으므로 반드시 주의*</returns>
    public static bool FindJObjectByKeyAndValue(JArray jArray, string key, string value, out (JObject, int) val)
    {
        int count = 0;
        foreach (JObject obj in jArray.Cast<JObject>())
        {
    ...
    /// <summary>
    /// 키(TYPE)의 값이 일치하는 Dictionary 반환
    /// </summary>
    /// <param name="jArray"></param>
    /// <param name="skillNameList"></param>
    /// <param name="data"></param>
    /// <returns></returns>
    public static bool FindJObjectByBuffDataKeyMultiple(JArray jArray, HashSet<string> nameList, out Dictionary<string, JObject> data)
    {
        data = new Dictionary<string, JObject>();

        foreach (JObject result in jArray.Cast<JObject>())
        {
            string type = result.ContainsKey(TYPE) ? result[TYPE]?.ToString() : null;

            if (!string.IsNullOrEmpty(type) && nameList.Contains(type))
            {
                data[type] = result;
                continue;
            }
    ...
    /// <summary>
    /// 키(변수)가 일치하는 Dictionary 반환
    /// </summary>
    /// <param name="jArray"></param>
    /// <param name="skillNameList"></param>
    /// <param name="data"></param>
    /// <returns></returns>
    public static bool FindJObjectByBuffDataKeyMultiple(JArray jArray, HashSet<string> nameList, out Dictionary<string, JObject> data, string key)
    {
        data = new Dictionary<string, JObject>();

        foreach (JObject result in jArray.Cast<JObject>())
        {
            string type = result.ContainsKey(key) ? result[key]?.ToString() : null;
    ...
    /// <param name="key"></param>
    /// <returns></returns>
    public static bool FindJObjectByKey(JArray jArray, string key, out JObject data)
    {
        var matchedObjects = jArray
                            .Cast<JObject>()
                            .Where(obj => obj.ContainsKey(key))
                            .ToList();

        if (matchedObjects.Count == 1)
        {
            data = matchedObjects[0]; // 기존과 동일
            return true;
        }
        else if (matchedObjects.Count > 1)
        {
            data = new JObject { ["multipleResults"] = new JArray(matchedObjects) }; // JArray를 JObject로 감싸서 반환
            return true;
        }
        else
        {
    ...
    /// <summary>
    /// 키가 일치하는 Dictionary 반환
    /// </summary>
    /// <param name="jArray"></param>
    /// <param name="skillNameList"></param>
    /// <param name="data"></param>
    /// <returns></returns>
    public static bool FindJObjectByKeyMultiple(JArray jArray, HashSet<string> skillNameList, out Dictionary<string, JObject> data)
    {
        data = new Dictionary<string, JObject>();

        foreach (JObject result in jArray.Cast<JObject>())
        {
            string? buffType = result.ContainsKey("buffType") ? result["buffType"]?.ToString() : null;
            string? debuffType = result.ContainsKey("debuffType") ? result["debuffType"]?.ToString() : null;

            if (!string.IsNullOrEmpty(buffType) && skillNameList.Contains(buffType))
            {
                data[buffType] = result;
                continue;
            }

            if (!string.IsNullOrEmpty(debuffType) && skillNameList.Contains(debuffType))
            {
                data[debuffType] = result;
                continue;
            }
        }
    ...
    /// <param name="data"></param>
    /// <returns></returns>
    public static bool FindJObjectByKeyMultipleByAttack(
        JArray jArray,
        HashSet<string> typeList,
        out Dictionary<string, JObject> data)
    {
        data = new Dictionary<string, JObject>();
        JArray debuffArray = new();
        JArray buffArray = new();

        foreach (JObject result in jArray.Cast<JObject>())
        {
            bool isHandled = false;

            if (result.ContainsKey("buffType"))
            {
                buffArray.Add(result);
                continue;
            }

            if (result.ContainsKey("debuffType"))
            {
                debuffArray.Add(result);
                continue;
            }

            foreach (string key in typeList)
            {
            ...
            //로직에 의미는 없으나 명시적인 의미로 추가
            if (isHandled) continue;
        }
        if (buffArray.Count > 0) data["buffList"] = new JObject { ["buffList"] = buffArray };
        if (debuffArray.Count > 0) data["debuffList"] = new JObject { ["debuffList"] = debuffArray };
    ...
    /// <param name="key"></param>
    /// <returns></returns>
    public static JObject FindJObjectByKey(JArray jArray, string key)
    {
        var matchedObjects = jArray
                            .Cast<JObject>()
                            .Where(obj => obj.ContainsKey(key))
        ...
        if (matchedObjects.Count == 1)
        {
            return matchedObjects[0]; // 기존과 동일
        }
        else if (matchedObjects.Count > 1)
        {
            return new JObject { ["multipleResults"] = new JArray(matchedObjects) }; // JArray를 JObject로 감싸서 반환
